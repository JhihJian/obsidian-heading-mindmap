...
导图视图采用上下分屏。上方画布在布局前使用隐藏测量节点复用真实主题、字体、徽标和换行 CSS，读取标题的实际 DOM 尺寸并加入安全余量；同一渲染样式下的测量结果会缓存，主题 CSS 变化后清空缓存并重新布局。树布局根据实测尺寸绘制节点和连线，长标题完整换行展示。工具栏提供缩放、适配全图和 100% 重置入口，视野 DOM 辅助逻辑负责按实测画布尺寸计算缩放、滚动区域和居中偏移。下方正文面板 DOM 和运行时模块根据当前选中节点显示阅读视图或源码编辑视图，内容使用面板可用宽度排版，也可最小化为标题栏以释放导图编辑空间。只读节点只能预览，不能编辑标题、正文或结构。
...
- `src/mindmap-toolbar-dom.ts`：导图工具栏 DOM 组装。负责标题、路径、缩放控制、列表项开关、添加文件节点入口和快捷键速查入口。
...
- `src/mindmap-viewport-runtime.ts`：导图视野运行时。负责把工具栏和滚轮缩放动作应用到实测画布尺寸，并同步滚动区域、居中偏移和持久化状态。
- `src/mindmap-node-measurer.ts`：节点尺寸测量层。负责用真实 DOM 和当前主题字体测量标题宽高、加入安全余量并缓存测量结果。
- `src/mindmap-node-display.ts`：节点显示元数据。集中提供不同节点类型的徽标文本，确保测量节点与正式节点一致。
- `src/body-pane-dom.ts`：正文面板标题栏、模式按钮、最小化按钮和拖拽条 DOM 组装。
- `src/body-pane-runtime.ts`：正文面板运行时。负责 Markdown 预览渲染、阅读模式双击进入编辑、CodeMirror 编辑器生命周期、正文图片粘贴附件写入和编辑器聚焦。
...
- `src/mindmap-shortcut-help.ts`、`src/mindmap-shortcut-help-modal.ts`：快捷键速查表数据和弹窗渲染。
...
- 文件节点展开的大纲只包含目标文件的真实标题节点，不包含目标文件的文档根节点。
...
- 添加文件节点的候选项应支持按路径检索；快捷键帮助和正文阅读模式双击编辑属于视图交互，不改变 Markdown 数据模型。
