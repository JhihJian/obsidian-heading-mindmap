...
const VIEW_TYPE_MINDMAP = "outline-mindmap-view";
const DEFAULT_MINDMAP_PATH = "Mindmaps/未命名思维导图.md";
...
export default class OutlineMindmapPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.registerView(
      VIEW_TYPE_MINDMAP,
      (leaf) => new OutlineMindmapView(leaf, this)
    );
    ...
    this.addCommand({
      ...
      name: "切换正文列表项在导图中展示",
      callback: () => {
        const view = this.app.workspace.getActiveViewOfType(OutlineMindmapView);
        if (!view) return;
  ...
  async activateView(): Promise<void> {
    ...
    const activeView = this.app.workspace.getActiveViewOfType(OutlineMindmapView);
  ...
  private async refreshViewsForFile(file: TFile): Promise<void> {
    ...
    for (const leaf of leaves) {
      ...
      if (view instanceof OutlineMindmapView && view.matchesFile(file.path)) {
        await view.reloadFromDisk();
      } else if (view instanceof OutlineMindmapView && view.usesExpandedFile(file.path)) {
  ...
  private async getActiveOrDefaultMindmapFile(): Promise<TFile> {
    const activeMindmap = this.app.workspace.getActiveViewOfType(OutlineMindmapView);
...
class OutlineMindmapView extends ItemView {
  ...
  constructor(leaf: WorkspaceLeaf, plugin: OutlineMindmapPlugin) {
    super(leaf);
    this.plugin = plugin;
  ...
  getDisplayText(): string {
    return this.currentFile?.basename ?? "Outline Mindmap";
  }
  ...
  private render(): void {
    ...
    this.destroyBodyEditor();
    container.empty();
    container.addClass("outline-mindmap-view");
    ...
    const toolbar = container.createDiv({ cls: "outline-mindmap-toolbar" });
    this.createToolbar(toolbar);

    const split = container.createDiv({ cls: "outline-mindmap-split" });

    this.canvasEl = split.createDiv({ cls: "outline-mindmap-canvas" });
  ...
  private createToolbar(toolbar: HTMLElement): void {
    const title = toolbar.createDiv({ cls: "outline-mindmap-toolbar-title" });
    ...
    const actions = toolbar.createDiv({ cls: "outline-mindmap-toolbar-actions" });
    const listToggleLabel = actions.createEl("label", { cls: "outline-mindmap-toolbar-toggle" });
  ...
  private renderCanvas(canvas: HTMLElement): void {
    ...
    const scrollArea = canvas.createDiv({ cls: "outline-mindmap-scroll-area" });
    ...
    const surface = scrollArea.createDiv({ cls: "outline-mindmap-surface" });
  ...
  private renderNode(surface: HTMLElement, node: MindNode, x: number, y: number): void {
    ...
    const header = el.createDiv({ cls: "outline-mindmap-node-header" });
    if (this.titleEditingNodeId === node.id) {
      const input = header.createEl("input", {
        type: "text",
        cls: "outline-mindmap-node-title-input"
      });
      ...
      const titleEl = header.createSpan({ text: node.title, cls: "outline-mindmap-node-title" });
    ...
    header.createSpan({ text: this.getNodeBadge(node), cls: "outline-mindmap-node-badge" });
  }
  ...
  private renderBodyPane(container: HTMLElement): void {
    ...
    const pane = container.createDiv({ cls: "outline-mindmap-body-pane" });
    const header = pane.createDiv({ cls: "outline-mindmap-body-header" });
    const heading = header.createDiv({ cls: "outline-mindmap-body-heading" });
    heading.createDiv({ text: node.title, cls: "outline-mindmap-body-title" });
    heading.createDiv({ text: this.getNodeBodyMeta(node), cls: "outline-mindmap-body-meta" });

    const actions = header.createDiv({ cls: "outline-mindmap-body-actions" });
    if (!readonly) {
      ...
      modeButton.buttonEl.addClass("outline-mindmap-body-mode-button");
    }
  ...
  private async renderMarkdownPreview(node: MindNode): Promise<void> {
    ...
    if (!markdown.trim()) {
      preview.empty();
      preview.createDiv({ text: "当前节点没有正文。", cls: "outline-mindmap-body-empty" });
      return;
    }
  ...
  private renderBodyPaneOnly(): void {
    ...
    const oldPane = split?.querySelector<HTMLElement>(".outline-mindmap-body-pane");
  ...
  private updateSelectionView(): void {
    ...
    surface.querySelectorAll<HTMLElement>(".outline-mindmap-node.is-selected").forEach((nodeEl) => {
  ...
  private shouldHandleDocumentShortcut(event: KeyboardEvent): boolean {
    ...
    return shouldHandleDocumentShortcutTarget({
      ...
      activeViewIsMindmap: this.plugin.app.workspace.getActiveViewOfType(OutlineMindmapView) === this,
...
class MarkdownFilePickerModal extends Modal {
  ...
  constructor(plugin: OutlineMindmapPlugin, currentFilePath: string | undefined, onChoose: (file: TFile) => void) {
    super(plugin.app);
    this.plugin = plugin;
...
function normalizePluginData(value: unknown): OutlineMindmapPluginData {
  if (!value || typeof value !== "object") {
    return { files: {}, expandListItems: false };
  }

  const files = (value as Partial<OutlineMindmapPluginData>).files;
  if (!files || typeof files !== "object") {
    return {
      files: {},
      expandListItems: Boolean((value as Partial<OutlineMindmapPluginData>).expandListItems)
    };
  }
